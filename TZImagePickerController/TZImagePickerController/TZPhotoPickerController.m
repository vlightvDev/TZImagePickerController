...
- (void)viewDidLoad {
    ...
    self.view.backgroundColor = [UIColor colorWithRed:24 / 255.0 green:20 / 255.0 blue:38 / 255.0 alpha:1];
...
- (void)titleButtonClick:(UIButton *)button
{
    if (!self.photoPickerTableView) {
        ...
        photoPickerTableView.selectedAlbumBlcok = ^(TZAlbumModel *_Nonnull model, UIButton *button) {
            ...
            weakSelf.models = [NSMutableArray arrayWithArray:model.models];
...
- (void)fetchAssetModels {
    ...
    dispatch_async(dispatch_get_global_queue(0, 0), ^{
        ...
        if (!tzImagePickerVc.sortAscendingByModificationDate && self->_isFirstAppear && self->_model.isCameraRoll) {
            [[TZImageManager manager] getCameraRollAlbumWithFetchAssets:YES
...
- (void)configCollectionView {
    if (!_collectionView) {
        ...
        _collectionView.backgroundColor = [UIColor colorWithRed:24 / 255.0 green:20 / 255.0 blue:38 / 255.0 alpha:1];
    ...
    if (@available(iOS 13.0, *)) {
        UIColor *divideLineDyColor = [UIColor colorWithDynamicProvider:^UIColor *_Nonnull (UITraitCollection *_Nonnull trainCollection) {
            if ([trainCollection userInterfaceStyle] == UIUserInterfaceStyleLight) {
                return [UIColor colorWithRed:rgb2
                                       green:rgb2
                                        blue:rgb2
                                       alpha:1.0];
            } else {
                CGFloat lineDarkRgb = 100 / 255.0;
                return [UIColor colorWithRed:lineDarkRgb
                                       green:lineDarkRgb
                                        blue:lineDarkRgb
                                       alpha:1.0];
            }
...
{
    _bottomToolBar.backgroundColor = [UIColor colorWithRed:24 / 255.0 green:20 / 255.0 blue:38 / 255.0 alpha:1];
...
- (void)viewDidLayoutSubviews {
    ...
    if (tzImagePickerVc.allowPickingOriginalPhoto) {
        ...
        _originalPhotoButton.frame = CGRectMake(0, 0, fullImageWidth + 56, 50);
        _originalPhotoLabel.frame = CGRectMake(fullImageWidth + 38, 0, 80, 50);
    }
...
- (void)doneButtonClick {
    ...
    for (NSInteger i = 0; i < tzImagePickerVc.selectedModels.count; i++) {
        ...
        __block TZAssetModel *model = tzImagePickerVc.selectedModels[i];
        if (model.asset.mediaType == PHAssetMediaTypeVideo) {
            if (model.asset.duration > tzImagePickerVc.maxVideoDuration) {
                error = @"文件超过时长限制";
            ...
            [[PHImageManager defaultManager] requestAVAssetForVideo:model.asset options:options resultHandler:^(AVAsset * _Nullable asset, AVAudioMix * _Nullable audioMix, NSDictionary * _Nullable info) {
                if ([asset isKindOfClass:[AVURLAsset class]]) {
                    ...
                    NSLog(@"%f",sizeValue);
                    if (sizeValue > tzImagePickerVc.maxVideoSize) {
                        error = @"文件超过大小限制";
                    }
                    model.dataSize = sizeValue;
                }
                dispatch_group_leave(imageGroup);
            }];
        } else {
            ...
            TZImageRequestOperation *operation = [[TZImageRequestOperation alloc] initWithAsset:model.asset
                ...
                if (photo) {
                    if (![TZImagePickerConfig sharedInstance].notScaleImage) {
                        photo = [[TZImageManager manager] scaleImage:photo
                                                              toSize:CGSizeMake(tzImagePickerVc.photoWidth, (int)(tzImagePickerVc.photoWidth * photo.size.height / photo.size.width))];
                    }

                    [photos replaceObjectAtIndex:i
                                      withObject:photo];
                }

                if (info) {
                    [infoArr replaceObjectAtIndex:i
                                       withObject:info];
                }

                [assets replaceObjectAtIndex:i
                                  withObject:model.asset];
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    cell.didSelectPhotoBlock = ^(BOOL isSelected) {
        ...
        if (isSelected) {
            ...
            if (tzImagePickerVc.isSelectAudio && model.asset.mediaType == PHAssetMediaTypeVideo) {
#ifdef HAVE_FDFullscreenPopGesture_H
                [CustomHUD showErrorHUD:@"动态不支持语音和视频同时发布"];
#endif
                return;
            }
            if (tzImagePickerVc.imagesVideoExclusion) {
                TZAssetModel *selectedModel = [tzImagePickerVc.selectedModels firstObject];
                BOOL shouldDone = true;
                if (selectedModel) {
                    if (selectedModel.asset.mediaType == PHAssetMediaTypeVideo) {
#ifdef HAVE_FDFullscreenPopGesture_H
                        [CustomHUD showErrorHUD:@"最多只能选择1个视频"];
#endif
                        shouldDone = tzImagePickerVc.maxVideoCount == 1;
                        if (model.asset.mediaType != PHAssetMediaTypeVideo) {
                            return;
                        } else if (tzImagePickerVc.selectedModels.count >= tzImagePickerVc.maxVideoCount) {
                            return;
                        }
                    } else {
                        shouldDone = tzImagePickerVc.maxImagesCount == 1;
                        if (model.asset.mediaType == PHAssetMediaTypeVideo) {
                            return;
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.item == [self getTakePhotoCellIndex]) {
        if (tzImagePickerVc.selectedModels.firstObject
            && tzImagePickerVc.selectedModels.firstObject.asset.mediaType == PHAssetMediaTypeVideo) {
#ifdef HAVE_FDFullscreenPopGesture_H
            [CustomHUD showErrorHUD:@"添加视频后，不可使用拍照功能"];
#endif
            return;
        }
...
- (void)takePhoto {
    ...
    if ((authStatus == AVAuthorizationStatusRestricted || authStatus == AVAuthorizationStatusDenied)) {
        ...
        UIAlertAction *settingAct = [UIAlertAction actionWithTitle:[NSBundle tz_localizedStringForKey:@"Setting"]
...
- (void)pushImagePickerController {
    ...
    if ([UIImagePickerController isSourceTypeAvailable:sourceType]) {
        ...
        if (self.presentedViewController) {
            [self.presentedViewController presentViewController:_imagePickerVc animated:YES completion:nil];
            return;
        }
...
- (void)refreshBottomToolBarStatus {
    ...
    if (tzImagePickerVc.allowPickingOriginalPhoto) {
        TZAssetModel *model = tzImagePickerVc.selectedModels.firstObject;
        if (model && model.type == TZAssetModelMediaTypeVideo) {
...
- (void)pushPhotoPrevireViewController:(TZPhotoPreviewController *)photoPreviewVc needCheckSelectedModels:(BOOL)needCheckSelectedModels {
    ...
    [photoPreviewVc setDoneButtonClickBlockCropMode:^(UIImage *cropedImage, id asset) {
        ...
        [strongSelf didGetAllPhotos:photos
                             assets:assets
                            infoArr:nil];
    }];
...
- (void)getSelectedPhotoBytes {
    ...
    TZImagePickerController *imagePickerVc = (TZImagePickerController *)self.navigationController;
    [[TZImageManager manager] getPhotosBytesWithArray:imagePickerVc.selectedModels
...
- (void)checkSelectedModels {
    ...
    if (!self.models || !self.models.count) return;
    ...
    if (self.isShowOnlyGif) {
        NSMutableArray *tempArray = NSMutableArray.new;
        [tempArray addObjectsFromArray:self.models];
        for (TZAssetModel *model in self.models) {
            if (model.type != TZAssetModelMediaTypePhotoGif && [tempArray containsObject:model]) {
...
- (void)callDelegate:(PHAsset *)asset isSelect:(BOOL)isSelect {
    ...
    [[TZImageManager manager] getPhotoWithAsset:asset
        ...
        if (isSelect) {
            [strongImagePickerVc.pickerDelegate imagePickerController:strongImagePickerVc
                                                       didSelectAsset:asset
                                                                photo:photo
                                                isSelectOriginalPhoto:strongSelf.isSelectOriginalPhoto];
        } else {
            [strongImagePickerVc.pickerDelegate imagePickerController:strongImagePickerVc
